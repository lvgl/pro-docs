---
title: Renesas
description: Integration guide for LVGL Pro Editor with Renesas development tools and e2studio IDE.
---
...
First, download and install the LVGL Editor from
[lvgl.io/pro](https://lvgl.io/pro#download).

LVGL provides an e2 Studio plugin to facilitate working with LVGL projects.
You can download it from 
[this link](https://github.com/lvgl/lvgl_pro/releases/download/v1.1.0/LVGL_Pro_e2.studio_plugin.v1.0.1.zip).
...
5. In the dialog that appears, click on "Local\..." and browse to the
 folder where you extracted the plugin.
6. Select the plugin folder, give the source a name (e.g. "LVGL Editor
 Launcher") and follow the prompts to complete the installation
 (approve any security warnings, accept the license agreement, and
 finish the installation).
7. Restart e2 Studio if prompted.

The plugin adds an LVGL Editor button to the toolbar, which launches the
LVGL Editor as a separate application, outside of e2 Studio:

<Figure
  src="/_static/images/renesas/e2studio_toolbar_with_editor_plugin.png"
  alt="e2 Studio Toolbar with LVGL Editor Plugin"
  caption="e2 Studio Toolbar with LVGL Editor Plugin"
/>
...
The LVGL Editor plugin will attempt to determine the currently active
project in the IDE. It picks the first project that:

1. Contains a Renesas `configuration.xml` file in its root
 folder. Or
2. Pick the project to which the currently active file (in the code
 editor) belongs.
...
Once the LVGL Editor is open, you can create and edit LVGL XML files as
usual. Use the Editor's export code functionality to generate C code,
which E2Studio will automatically detect and include in its build.
